...
for where it is: on `self`, or by a name the file defines, in the file
itself; on a module, in that module; on anything else, in the other files.
By name alone, `targets.unfolding` would be `elaborate.py`'s method of the
same name, and `work.normalize` in `elaborate.py` would be nothing.
...
def hits(call, known):
    ...
    `known` is the file's own such functions, those of the others, and every
    name the file defines. A call on `self`, or of a name the file defines,
    is the file's own; any other is another file's, and is looked for there.
    ...
    """
    own, others, defined, modules = known
...
def declining(tree, others=None):
    """The names of the functions in a file that can give back a decline.

    `others` is what declines in each other file, by module name. A function
    declines if it gives back what a declining one gave it, so the set is
    closed by going round until it stops growing.
    """
    others = others or {}
    by_name = {}
    ...
    while growing:
        ...
        for name, defs in by_name.items():
            if name not in own and any(
                    declines(fn, (own, others, set(by_name), modules))
                    for fn in defs):
                own.add(name)
                growing = True
    return own
...
def across(paths):
    ...
    while growing:
        ...
        for p, tree in trees.items():
            found = declining(tree, others(p))
            if found != own[p]:
                own[p], growing = found, True
    return {p: others(p) for p in paths}
...
def main(argv):
    ...
    for path in wanted:
        ...
        for line, owner, inner, outer in sorted(sites(path, others[path])):
