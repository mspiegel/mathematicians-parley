...
it. Break the elaborator, leave the built files alone, and all ten stages
pass while no proof elaborates at all. `build.py` is what compares the two —
it says `30 built, 0 changed` — and the gate is what checks the result.

Ten things: the lint settings in `ruff.toml`, that no caller hands on a
...
names, that a compressed proof is the proof it was made from, that no
elaborated proof takes a step as stated that `ELABORATION.md` does not
record, and a verifier over every proof the elaborator has written. Any one
of them failing fails the gate.
...
file's header is read by nobody.
